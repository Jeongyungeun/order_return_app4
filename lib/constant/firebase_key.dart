const COL_USERS = 'users';
const COL_CARDS = 'cards';
const COL_USER_CARDS = 'user_card';
const COL_CHATROOMS = 'chatrooms';
const COL_CHATS = 'chats';


const DOC_GEOFIREPOINT = 'geoFirePoint';
const DOC_PHONENUMBER = 'phoneNumber';
const DOC_CREATEDDATE = 'createdDate';
const DOC_ISPHARMACIST = 'ispharmacist';

const DOC_CARDKEY= 'cardKey';
const DOC_USERKEY= 'userKey';
const DOC_NAME= 'name';
const DOC_COMPANY= 'company';
const DOC_CONTACTIMAGEURL = 'contactimageurl';


const DOC_YOURKEY = 'yourKey';
const DOC_LASTMSG = 'lastMsg';
const DOC_LASTMSGUSERKEY = 'lastMsgUserKey';
const DOC_LASTMSGTIME = 'lastMsgTime';
const DOC_LSTMSGUSERKEY = 'lstMsgUserKey';
const DOC_CHATROOMKEY = 'chatroomKey';

const DOC_ORDERPRODUCT = 'orderProduct';
const DOC_ORDERNUM = 'orderNum';
const DOC_ORDERMSG = 'orderMsg';

const DOC_MSG= 'msg';
